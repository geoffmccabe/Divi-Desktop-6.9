...
import { fitCollider, placeCollider, colliderBound, noseOf } from "./shipCollider";
...
function fighter(): THREE.Object3D {
  ...
  /* Longer than it is wide, and the wings set back from the nose, so "along the
     ship" is unambiguous and the front slices are genuinely narrower. */
...
//     visibly cleared the ship took their shield off, so that is what is asked.
{
  ...
  /* The fighter is 5 long, 3.5 across the wings at z = +1.2, and its fuselage
     is half a unit wide. A ball round the whole thing would be about 2.6. */
...
// 3. The nose is at the front, which is where the guns go.
{
  ...
  /* The wings sit behind the middle, so the narrow end — the nose — is the one
     at negative z, and it has to be a real distance from the centre. */
  ok("the nose is at the narrow end", nose.z < 0, `z ${nose.z.toFixed(2)}`);
