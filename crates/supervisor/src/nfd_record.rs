...
pub enum NfdRecord {
    /// First appearance of an NFD. arweave_ptr -> content bundle; content_hash is
    /// SHA-256 of the plaintext (doubles as a proof-of-existence).
    Mint { arweave_ptr: String, content_hash: String, flags: u8 },
...
pub fn encode_mint(arweave_ptr: &str, content_hash: &str, flags: u8) -> Result<String, String> {
...
pub fn parse(script_hex: &str) -> Option<NfdRecord> {
    ...
    match subtype {
        SUB_MINT if body.len() >= 130 => Some(NfdRecord::Mint {
            arweave_ptr: body[0..64].to_string(),
            content_hash: body[64..128].to_string(),
            flags: u8::from_str_radix(&body[128..130], 16).ok()?,
        }),
...
mod tests {
    ...
    fn mint_roundtrip() {
        let a = "aa".repeat(32);
        let h = "bb".repeat(32);
        let script = op_meta_script(&encode_mint(&a, &h, 0x01).unwrap());
        assert_eq!(
            parse(&script),
            Some(NfdRecord::Mint { arweave_ptr: a, content_hash: h, flags: 1 })
        );
    }
    ...
    fn encode_validates_lengths() {
        assert!(encode_mint("short", &"bb".repeat(32), 0).is_err());
